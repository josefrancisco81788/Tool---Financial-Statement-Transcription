#!/usr/bin/env python3
"""
Test script for uploading files to the Financial Statement Transcription API
"""
...
def test_file_upload(file_path, output_format="csv"):
    """
    ...
    if file_size < 5 * 1024 * 1024:  # Less than 5MB
        ...
        print("⚡ Using synchronous endpoint (file < 5MB)")
    else:
        endpoint = f"{API_BASE_URL}/api/v1/extract-financial-data"
        print("🔄 Using asynchronous endpoint (file >= 5MB)")
...
def list_sample_files():
    """List sample files in the current directory"""
    ...
    if files:
        for i, file in enumerate(files, 1):
            ...
            print(f"{i}. {file.name} ({size / 1024:.1f} KB)")
        return files
    else:
        print("❌ No PDF or image files found in current directory")
        print("💡 Please add a sample file to test with")
        return []

def main():
    """Main test function"""
    print("🧪 Financial Statement Transcription API - File Upload Test")
    print("=" * 70)
    ...
    if not files:
        ...
        print("3. Or specify a file path manually")
        return
    
    # Let user choose a file
    print(f"\n📋 Choose a file to test (1-{len(files)}) or enter a custom path:")
